...
int main(c_int argc, c_str_arr argv) {
    ...
    if (args_error.error_code != ILLEGAL_FLAG) {
        ...
        permission_denied = mx_print_files_and_directories(entries_list, &flags, mx_list_size(args_error.invalid_entry_names_list));
    ...
    return args_error.error_code || permission_denied ? 1 : 0;
}
