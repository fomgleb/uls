...
static void print_entries(t_list *entries_list, size_t column_spacing, ushort terminal_width, bool print_newline_in_the_end) {
    ...
    for (ushort y = 0; y < number_of_rows; y++) {
        for (ushort x = 0; x < number_of_columns; x++) {
            ...
            if (converted_index < number_of_entries) {
                ...
                mx_printnchar_if(x + 1 != number_of_columns, ' ', column_width - printing_string_length);
            }
        }
        mx_printchar('\n');
    }
...
static void print_contents_of_directories(t_list *directories_list, c_size_t column_spacing, c_ushort terminal_width, c_bool print_names_of_dirs) {
    for (t_list *i = directories_list; i != NULL; i = i->next) {
        ...
        print_entries(directory->entries_list, column_spacing, terminal_width, i->next != NULL);
    }
}

static void print_directory_content_recursively(t_entry *directory, c_size_t column_spacing, c_ushort terminal_width, bool print_dir_name, bool print_newline_in_the_beginning) {
    mx_printchar_if(print_newline_in_the_beginning, '\n');
    mx_print_two_strings_if(print_dir_name, directory->relative_path, ":\n");
    print_entries(directory->entries_list, column_spacing, terminal_width, false);

    t_files_dirs files_dirs = mx_separate_entries(directory->entries_list);
    for (t_list *i = files_dirs.dirs_list; i != NULL; i = i->next) {
        print_directory_content_recursively((t_entry *)i->data, column_spacing, terminal_width, true, true);
    }
}

static void print_contents_of_directories_recursively(t_list *directories_list, c_size_t column_spacing, c_ushort terminal_width, bool print_dir_name) {
    for (t_list *i = directories_list; i != NULL; i = i->next) {
        t_entry *directory = (t_entry *)i->data;
        print_directory_content_recursively(directory, column_spacing, terminal_width, print_dir_name, false);
...
void mx_print_files_and_directories_in_columns(t_list *entries_list, c_bool colorized, c_bool recursively) {
    ...
    print_entries(files_dirs.files_list, column_spacing, terminal_width, files_dirs.dirs_list != NULL);
    if (recursively) {
        print_contents_of_directories_recursively(files_dirs.dirs_list, column_spacing, terminal_width, files_dirs.total_entries_count > 1);
    } else {
        print_contents_of_directories(files_dirs.dirs_list, column_spacing, terminal_width, files_dirs.total_entries_count > 1);
    }
}
